 ...
 * @param {Object} container
 */
_.dealEdgeValue = (itemInfo, styleInfo, container, zones) => {
  if (styleInfo.hasOwnProperty('left') && styleInfo.left < 0) {
    console.log('改变位置left<0', styleInfo.hasOwnProperty('left') && styleInfo.left < 0, styleInfo)
    styleInfo.left = 0
    styleInfo.width = itemInfo.width + itemInfo.left
  }

  if (styleInfo.hasOwnProperty('top') && styleInfo.top < 0) {
    console.log('改变位置top<0', styleInfo.hasOwnProperty('top') && styleInfo.top < 0, styleInfo)
    styleInfo.top = 0
  ...
  if (!styleInfo.hasOwnProperty('left') && styleInfo.hasOwnProperty('width')) {
    if (itemInfo.left + styleInfo.width > container.width) {
      console.log('宽度变化后大于父级宽度', itemInfo.left + styleInfo.width > container.width)
      styleInfo.width = container.width - itemInfo.left
      console.log(container.width, styleInfo.width)
    }
  }

  if (!styleInfo.hasOwnProperty('top') && styleInfo.hasOwnProperty('height')) {
    if (itemInfo.top + styleInfo.height > container.height) {
      console.log('高度变化后大于父级宽度', itemInfo.top + styleInfo.height > container.height)
      styleInfo.height = container.height - itemInfo.top
      console.log(container.height, styleInfo.height)
    }
  }
  ...
  if (zones.length > 1) {
    ...
    for (let i = 0, len = currentzones.length - 1; i < len; i++) {
      if (_.handleEgdeCollisions(currentzones[i], current)) {
        console.log('碰撞', _.handleEgdeCollisions(currentzones[i], current))
        return itemInfo
      }
    }
  }
...
_.dealBR = (itemInfo, moveX, moveY, minLimit = _.MIN_LIMIT) => {
  ...
  let width = itemInfo.width + moveX
  let height = itemInfo.height + moveY
  // console.log('dealBR', width, itemInfo.width, height, itemInfo.height)
  if (width >= Math.min(minLimit, itemInfo.width)) {
